...
#include <zxing/qrcode/encoder/BlockPair.h>
...
namespace zxing {
    namespace qrcode {
        ...
        std::array<int, 96> Encoder::ALPHANUMERIC_TABLE = {
        ...
        std::string Encoder::DEFAULT_BYTE_MODE_ENCODING = "ISO-8859-1";

        // The mask penalty calculation is complicated.  See Table 21 of JISX0510:2004 (p.45) for details.
        // Basically it applies four rules and summate all penalties.
        int Encoder::calculateMaskPenalty(std::shared_ptr<ByteMatrix> matrix) {
         ...
         */
        std::unique_ptr<QRCode> Encoder::encode(
        ...
        std::unique_ptr<QRCode> Encoder::encode(
            ...
            std::string encoding =
                    hints == nullptr ? "" : hints->get(EncodeHintType::CHARACTER_SET);
            ...
            Ref<BitArray> headerBits(new BitArray());
            ...
            // main payload yet.
            Ref<BitArray> dataBits(new BitArray());
            ...
            Ref<BitArray> headerAndDataBits(new BitArray());
            headerAndDataBits->appendBitArray(headerBits);
            // Find "length" of main segment and write it
            int numLetters = mode == &Mode::BYTE ? dataBits->getSizeInBytes() : content.size();
            ...
            std::unique_ptr<QRCode> qrCode(new QRCode());
            ...
            std::shared_ptr<ByteMatrix> matrix(new ByteMatrix(dimension, dimension));
        ...
        Mode *Encoder::chooseMode(const std::vector<char> &content) {
            return chooseMode(content, std::string());
        }
         ...
         */
        void Encoder::terminateBits(int numDataBytes, Ref<BitArray> bits) {
            int capacity = numDataBytes << 3;
            if (bits->getSize() > capacity) {
                throw WriterException("data bits cannot fit in the QR Code" /*+ bits->getSize() + " > " +
                                          capacity*/);
            }
         ...
         */
        Ref<BitArray>
        Encoder::interleaveWithECBytes(Ref<BitArray> bits,
                                       int numTotalBytes,
                                       int numDataBytes,
                                       int numRSBlocks) {
            ...
            for (int i = 0; i < numRSBlocks; ++i) {
                ArrayRef<int> numDataBytesInBlock(1);
                ArrayRef<int> numEcBytesInBlock(1);
                ...
                ArrayRef<int8_t> dataBytes(size);
                bits->toBytes(8 * dataBytesOffset, dataBytes, 0, size);
                ArrayRef<int8_t> ecBytes = generateECBytes(dataBytes, numEcBytesInBlock[0]);
                blocks.push_back(BlockPair(dataBytes, ecBytes));
            ...
            for (int i = 0; i < maxNumDataBytes; ++i) {
                for (BlockPair &block: blocks) {
                    ArrayRef<int8_t> dataBytes = block.getDataBytes();
                    if (i < dataBytes->size()) {
            ...
            for (int i = 0; i < maxNumEcBytes; ++i) {
                for (BlockPair &block: blocks) {
                    ArrayRef<int8_t> ecBytes = block.getErrorCorrectionBytes();
                    if (i < ecBytes->size()) {
            ...
            if (numTotalBytes != result->getSizeInBytes()) {  // Should be same.
                throw WriterException("Interleaving error: "/* + numTotalBytes + " and " +
        ...
        ArrayRef<int8_t>
        Encoder::generateECBytes(ArrayRef<int8_t> dataBytes, int numEcBytesInBlock) {
            ...
            ArrayRef<int8_t> ecBytes(numEcBytesInBlock);
         ...
         */
        void Encoder::appendModeInfo(Mode *mode, Ref<BitArray> bits) {
         ...
         */
        void Encoder::appendLengthInfo(int numLetters,
                                       Version *version,
                                       Mode *mode,
                                       Ref<BitArray> bits) {
         ...
         */
        void Encoder::appendBytes(const std::vector<char> &content,
                                  Mode *mode,
                                  Ref<BitArray> bits,
                                  std::string encoding) {
        ...
        void Encoder::appendNumericBytes(const std::vector<char> &content,
                                         Ref<BitArray> bits) {
        ...
        void
        Encoder::appendAlphanumericBytes(const std::vector<char> &content,
                                         Ref<BitArray> bits) {
            int length = content.size();
            int i = 0;
            while (i < length) {
        ...
        void Encoder::append8BitBytes(const std::vector<char> &content,
                                      Ref<BitArray> bits,
                                      std::string encoding) {
            /*byte[] bytes;
            try {
        ...
        void Encoder::appendECI(common::CharacterSetECI *eci, Ref<BitArray> bits) {
